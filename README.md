...
> `.mp4` 放入 `assets/` 后再执行 `node build.cjs`。
...
DSH 只服务插件的 `client.js` 一个文件，因此 `build.cjs` 会把 `assets/`、`GIF/`、
`music/` 里的素材以 base64 内嵌进 `lib/client.js`。
...
- **素材**：
  ...
  - **网络壁纸资源**（部分静态图片与全部 mp4 动态壁纸，如「哲风壁纸」等第三方来源）
    **不随仓库分发**。动态壁纸为可选功能：使用者自行将已授权/原创的 `.mp4` 放入
    `assets/` 后运行 `build.cjs` 即可启用；没有 mp4 时主题会自动回退为静态壁纸，
    其余功能不受影响。
