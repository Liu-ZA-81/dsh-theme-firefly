	...
	factory: (require) => {
		...
		function startWallpaper() {
			...
			panel.innerHTML =
				...
				'<div class="ff-bg-line"><span class="ff-bg-label">模式</span>' +
					'<button class="ff-bg-seg" data-mode="switch" type="button">切换</button>' +
			...
			modeBtns.switch.addEventListener("click", () => { setMode("switch"); doSwitch(); });
			...
			return () => {
				clearRandom();
				bg.remove(); shade.remove(); btn.remove(); panel.remove();
			};
		}
		...
		function startMusic() {
			...
			card.innerHTML =
				...
				'<button class="ff-music-btn" data-act="prev" title="上一首">⏮</button>' +
				'<button class="ff-music-btn ff-music-play" data-act="play" title="播放/暂停">▶</button>' +
				'<button class="ff-music-btn" data-act="next" title="下一首">⏭</button>' +
				'<button class="ff-music-btn ff-music-mode" data-act="mode" title="循环模式">列表</button>' +
				'</div>';
		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				const onEsc = (e) => {
					...
					document.querySelectorAll(".ff-bg-panel.open, .ff-amb-menu.open").forEach((el) => el.classList.remove("open"));
